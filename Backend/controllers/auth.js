...
const login = async (req, res, next) => {
 try {
   ...
   //create jwts
   const accessToken = jwt.sign({ userId: user._id }, tokenSecret, { expiresIn: "24h" });

   //create new refresh token
   const newRefreshToken = jwt.sign({ userId: user._id }, refreshSecret, { expiresIn: "24h" });
...
const handleRefreshToken = async (req, res) => {
     ...
     jwt.verify(oldRT, refreshSecret, (err, decoded) => {
          if (err || user._id.toString() !== decoded.userId) return res.status(StatusCodes.FORBIDDEN);
     });

     //generate new tokens
     const accessToken = jwt.sign({ userId: user._id }, accessSecret,{ expiresIn: '24h' });  

     const newRT = jwt.sign({ userId: user._id },refreshSecret, { expiresIn: '12h' });
